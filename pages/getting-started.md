...
**Note:** A similar attack is possible even *without* fullscreen mode. Since a compromised domain can draw pixels within its own windows however it likes, it could draw a fake password prompt, for example, which appears to have a different colored border so that it looks like it belongs to a different domain. This is why you should always drag such prompts off of other windows (or use some other means of manipulating the windows) to ensure that they belong to the domains to which they appear to belong.
